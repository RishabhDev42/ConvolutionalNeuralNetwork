...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        num_inp_channels = num_input_channels

        self.convolutional_layers = []
        for i in range(self.nlayers):
            self.convolutional_layers.append(
                Conv1d(num_inp_channels,num_channels[i],kernel_sizes[i],strides[i],0,conv_weight_init_fn,bias_init_fn))
            num_inp_channels = num_channels[i]
