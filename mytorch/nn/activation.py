...
class GELU:
    ...
    def forward(self,Z):
        self.Z = Z
        self.A = 0.5*(Z*(1+scipy.special.erf(self.Z/np.sqrt(2))))
        return self.A

    def backward(self, dLdA):
        dLdZ = dLdA * (0.5*(1+scipy.special.erf(self.Z/np.sqrt(2)))+(self.Z/np.sqrt(2*np.pi)*np.exp(-0.5*np.power(self.Z,2))))
        return dLdZ
...
class Softmax:
    ...
    def forward(self, Z):
        ...
        """
        self.A = np.exp(Z - np.max(Z, axis=1, keepdims=True)) / np.sum(np.exp(Z - np.max(Z, axis=1, keepdims=True)), axis=1, keepdims=True)
        return self.A
    ...
    def backward(self, dLdA):
        ...
        for i in range(N):
            ...
            for m in range(C):
                for n in range(C):
                    J[m, n] = np.where(m==n,self.A[i,m]*(1-self.A[i,m]),-self.A[i,m]*self.A[i,n])  # TODO
